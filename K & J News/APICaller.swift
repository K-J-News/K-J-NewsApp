...
public func getUserSearch(lang: String, country: String, query: String, completion: @escaping (Result<[Article], Error>) -> Void){
    guard !lang.trimmingCharacters(in: .whitespaces).isEmpty else{
        return
    }
    guard !country.trimmingCharacters(in: .whitespaces).isEmpty else{
    ...
    let urlString = ("https://newsapi.org/v2/top-headlines?sortedBy=popularity&apiKey=71105b4a980545dfa03385349ddad2a4&q=\(query)&language=\(lang)&country=\(country)")
    print(urlString)
    guard let url = URL(string: urlString) else{
        return
    }
